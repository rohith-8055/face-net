...
import com.ml.quaterion.facenetdetection.MainActivity.Companion.rowcount

import com.ml.quaterion.facenetdetection.MainActivity.Companion.sheet2
...
class FrameAnalyser( context: Context ,
    ...
    public fun updateAttendance(name: String){

        for (row in sheet2){
            val nameCell = row.getCell(0)
            if (nameCell.stringCellValue == name){
                val scoreCell = row.getCell(1)
                scoreCell.setCellValue("Present")
                break
            }
        }
            ...
            for (face in faces) {
                try {
                    ...
                    if (maskLabel == maskDetectionModel.NO_MASK) {
                        ...
                        updateAttendance(bestScoreUserName)
